...
class Overlay:
    ...
    def _build_settings(self):
        ...
        box.addWidget(self._hint(
            "写那三条候选，并判断意图、紧张度、给候选排序——一次调用全做完。"
            "OpenAI / Anthropic / Gemini 三种接口都走各自官方 SDK。"
    ...
    def _model_group(self, box, title, kind, table):
        """一组「来源 / 密钥 / 模型」控件。table 是 core/providers.py 里那张表。
        上游这里有两组（判断一组、起草一组）；判断合进了同一次调用，只剩起草这一组。"""
        group = SimpleNamespace(kind=kind, table=table, ids=list(table),
                                keyTitle="起草",
                                stored_key=settings.llm_key)
        heading = QHBoxLayout()
