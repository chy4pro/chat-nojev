...
这一版暂时关掉：这个分支还没有自己的 Release 页，而上游的版本号跟这里的代码不是一回事，
查上游只会把用户导去装另一个程序。等这个分支有了自己的仓库，把 _API 指过去、_REPO 填上即可。"""
from __future__ import annotations

import json
import urllib.request

_REPO = ""  # 例如 "owner/chat-nojev"；留空 = 不检查更新
_API = f"https://api.github.com/repos/{_REPO}/releases/latest" if _REPO else ""
...
def check_latest(current: str, timeout=6) -> tuple[str, str] | None:
    ...
    if not _API:  # 还没有自己的 Release 页，不去查别人的
        return None
