...
json_object=True 是各家的 JSON 模式（OpenAI 的 response_format / Gemini 的 response_mime_type）；
Anthropic 的 Messages API 没有等价开关，那家只能靠提示词 + 调用方的容错解析。
...
try:  # 当模块导入 / 当脚本直接跑 都能用
    from .errors import JevError, _fail
except ImportError:
    from errors import JevError, _fail
...
# 认 JSON 模式的协议。Anthropic 不在里面：它要 JSON 得走 tool use，那是另一套请求形状
JSON_MODE_PROTOCOLS = ("openai", "gemini")
...
def chat(protocol: str, base_url: str | None, api_key: str, model: str, system: str,
    ...
    json_object: 让接口只吐 JSON 对象。见 JSON_MODE_PROTOCOLS——anthropic 没有这个开关，传了也忽略。
                 有些 OpenAI 兼容的第三方地址不认这个字段（会 400/422），调用方要准备好退回去重发一次。
    """
...
def _gemini(base_url, api_key, model, system, user_turns, temperature, max_tokens,
            thinking, timeout, json_object=False) -> str:
    try:
        ...
        config = types.GenerateContentConfig(
            ...
            **({"response_mime_type": "application/json"} if json_object else {}))
...
if __name__ == "__main__":
    ...
    # JSON 模式：不传就一个字段都不加，传了才有 response_format
    ...
    assert seen["gemini.call"]["config"].response_mime_type is None  # 默认不开 JSON 模式
    chat("gemini", "", "k", "gemini-2", "S", ["U"], json_object=True)
    assert seen["gemini.call"]["config"].response_mime_type == "application/json"
