...
提示词，那里是这些文字唯一的家。OpenAI / Gemini 两种协议会顺带打开各自的 JSON 模式
（`response_format` / `response_mime_type`）；地址不认这个字段（400/422）就脱掉重发一次。
Anthropic 没有等价开关，只能靠提示词。

解析一律「读不出来就整条丢掉」：标签不在题目里的判断字段直接不进结果（界面显示「暂未判断」），
分数超范围夹回 0–9，不是数字就丢；排序整块没读出来就退回第一条、分记 0——跟上游收到垃圾时一模一样。
点名和最高分打架时**以点名为准**（上游也是这样：界面把点名那条强制排第一，分只用来给其余的排序）。
`tools/offline_check.py` 把这些坏形状都喂一遍，不联网。
