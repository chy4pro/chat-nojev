...
> 这一份是 chat-nojev 的改版：判断和起草合成一次普通模型调用，没有单独的判断模型。下面的文字是上游的，凡是说「调 Jev 判断」「判断接口」的地方，现在都是那一次调用里的事；题目原文和档位没变。

挂在任意聊天 App 旁边（微信、飞书已适配）的非侵入式助手：读到对方最新消息 → 一次调用拿到判断和 3 条候选回复（按模型给的分排序）→ 人一键填入微信输入框。**发送永远由人手动点，程序不自动发。**
...
- 模型客户端分两路：`jev/ReplyClient`（判断 + 候选回复，一次调用；答案由 `jev/MergedAnswer` 翻译成 `Analysis`）、`jev/VisionClient`（视觉，OCR 用），共用 `jev/HttpJson`；配置在 `core/Prefs`（`reply*` / `vision*` 字段），旧密钥一次性迁移到 `reply_key`，标记位 `prefs_migrated_v13`。
...
- ~~Jev = TypeSafe 的判断模型~~（这一版不再调它）。题目原文留在 `jev/JevQuestions.kt`，由 `renderJudgmentSpec()` 铺进那一次 `/chat/completions` 的提示词；答案形状仍是 noul / choice / score，`jev/MergedAnswer` 把它们搬进 `Analysis`，**只搬不校验**（下游本来就防着判断模型吐垃圾，再校验一次反而跟上游不一样）。与上游逐字段的比对见 `docs/EQUIVALENCE.md`。
- 题目沿用英文原文（上游判断模型的主训练语言是英文），聊天内容保留中文；3 道 choice 题要求模型用**对话那门语言**写一句短语作答，悬浮窗原样显示，不再有英文标签→中文的对照表。
