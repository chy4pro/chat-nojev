...
class ReplyClient(private val prefs: Prefs) {
    ...
    fun draftAndJudge(snapshot: ChatSnapshot, relationship: String, ctx: ChatContext? = null): Analysis {
        ...
        return try {
            ...
            val user = knowledgeBlock(relationship, ctx) +
                "关系：$relationship\n\n最近对话：\n$convo\n\n" +
                "请给出那一个 JSON 对象：replies 恰好 3 条，best_reply 点名其中一条，" +
     ...
     * must contain, then the judgment questions laid out verbatim by
     * [JevQuestions.renderJudgmentSpec] (their wording lives only there).
     */
    private fun system(): String =
        "你是中文即时通讯回复助手。读完整段对话，写 3 条候选回复，" +
