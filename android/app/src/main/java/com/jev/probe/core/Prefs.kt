...
/**
 * App-private config store. Holds the two API routes (reply / vision), the
 * relationship description used in the analysis prompt, the conversation
 * whitelist, plus the context (D stage) and OCR (B stage) switches.
 *
 * Upstream had three routes: a judgment model answered the 7 questions and
 * ranked the candidates on its own endpoint, with its own provider, key and
 * model. One ordinary model does both jobs here, so the reply route is the only
 * one analysis needs and the judge route is gone.
 *
...
class Prefs(context: Context, prefsName: String = PREFS_MAIN) {
    ...
    /**
     * Carry an existing key over to the one route that is left. v1.2 kept it in
     * `openrouter_key`, v1.3 in `judge_key`; either becomes the reply key when
     * that is still empty, so an upgrading user does not have to retype it.
     * `reply_model` keeps its old storage key and carries over untouched.
     */
    private fun migrateIfNeeded() {
        if (sp.getBoolean(K_MIGRATED_V13, false)) return   // runs exactly once
        val legacy = (sp.getString(K_LEGACY_JUDGE_KEY, "") ?: "")
            .ifBlank { sp.getString(K_LEGACY_KEY, "") ?: "" }
        val current = sp.getString(K_REPLY_KEY, "") ?: ""
        val e = sp.edit().putBoolean(K_MIGRATED_V13, true)
        if (current.isBlank() && legacy.isNotBlank()) {
            e.putString(K_REPLY_KEY, legacy)
            Log.i(TAG, "prefs migrated replyKey.len=${legacy.length}")
        } else {
            Log.i(TAG, "prefs migrated replyKey.len=${current.length} (no legacy key to copy)")
        }
        e.apply()
    }
    ...
    /** The one key analysis needs. */
    var replyKey: String
        get() = sp.getString(K_REPLY_KEY, "") ?: ""
        set(v) = sp.edit().putString(K_REPLY_KEY, v.trim()).apply()

    /** The model that writes the candidates and answers the judgment questions. */
    ...
    /** The reply route's key. Kept as a function so the call sites that asked
     *  for the effective key (it used to fall back to the judge key) read the
     *  same way. */
    fun effectiveReplyKey(): String = replyKey

    /** Vision route key, falling back to the reply key. */
    fun effectiveVisionKey(): String = visionKey.ifBlank { replyKey }
    ...
    /** Readiness gate: the one route analysis needs must be configured. */
    fun hasKey(): Boolean = replyKey.isNotBlank()
    ...
    companion object {
        ...
        private const val K_LEGACY_KEY = "openrouter_key"        // v1.2
        private const val K_LEGACY_JUDGE_KEY = "judge_key"       // v1.3's judge route
