...
> **这一份是 chat-nojev 的改版。** 上游（[jev-chat-jarvis](https://github.com/jev-chat/jev-chat-jarvis)）
> 把 7 道判断题和候选排序交给专门的判断模型（TypeSafe Jev），起草另算一次调用；这里把它们合成一次——
> 同一个语言模型写 3 条候选、答那 7 道判断题、点名哪条最好并给每条一个分。题目原文、选项集合、
> 0–9 的紧张度档位一个字没改（见 `jev/JevQuestions.kt`），悬浮窗读到的 `Analysis` 字段也一模一样，
> **只是产出方换了**。因此 `confidence` / `probabilities` 现在是模型的自评，不是上游那种校准过的概率。
> 配置上少了一路接口：分析只认「模型接口」那一把 key。逐字段与上游比对的结果见
> [`docs/EQUIVALENCE.md`](docs/EQUIVALENCE.md)。
>
> 这份仓库不发 APK（上游的 `apk/` 没有复制过来），也没有在真机上跑过；下面凡是提到装包和真机实测的
> 段落，说的都是上游那一版。
...
<td align="center"><img src="docs/images/settings.png" width="300" alt="设置页" /><br/><sub>设置页（上游截图：那时还是判断 / 回复 / 视觉三张卡，这一版只剩模型 / 视觉两张）</sub></td>
</tr>
</table>

## 为什么用它

- **它先判断，再写字。** 大多数工具直接让模型编一句回复。这里同一次调用里先答完「对方真实意图、危险等级、该不该马上回」那 7 道题，再据此给出 3 条候选（上游是判断模型单独答题，题目和档位一样）。
...
**1. 装包。** 这一份改版不发 APK，也没有 `apk/` 目录：要装得自己用 Android SDK 打一个（见下面「构建与目录结构」）。上游签好名的包在
[Releases](https://github.com/jev-chat/jev-chat-jarvis/releases)，那是**上游**那一版，不含这里的改动。
...
- 面板一次画完（上游是判断先回、候选后补，中间有「生成中…」；合成一次之后没有这个中间态）。
...
- **翻译**：`jev/MergedAnswer.kt` 只把模型给的值搬进上游那三种答案形状（noul / choice / score）和排序块，再跑上游原封不动的那几个解析函数；不查标签表、不夹范围、不归一，也不替模型编值。为什么是这个标准，见 `docs/EQUIVALENCE.md`。
...
- `tools/jev/` — 上游判断模型的题目集与校准脚手架（Python，原样留着：题目文字的出处在这里，但这一版不再调那个接口）
- `docs/` — 设计与验收文档，外加本版的 `EQUIVALENCE.md`（与上游逐字段比对的标准和结果）
...
- **中文**：题目沿用上游的英文原文（判断模型的主训练语言是英文），聊天内容保留中文；3 道 choice 题改成让模型用对话那门语言写一句短语作答，因此面板上的用词不再跟上游逐字相同。
- **没有真机验证**：这一版只在容器里编译和做了差分测试，没装到手机上跑过，也没打过包。
