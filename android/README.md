...
> **这一份是 chat-nojev 的改版，脱胎自 [jev-chat-jarvis](https://github.com/jev-chat/jev-chat-jarvis)。**
> 它把 7 道判断题和候选排序交给专门的判断模型（TypeSafe Jev），起草另算一次调用；这里把它们合成一次——
> 同一个语言模型写 3 条候选、答那 7 道判断题、点名哪条最好并给每条一个分。题目原文、选项集合、
> 0–9 的紧张度档位一个字没改（见 `jev/JevQuestions.kt`），悬浮窗读到的 `Analysis` 字段也一模一样，
> **只是产出方换了**。因此 `confidence` / `probabilities` 现在是模型的自评，不是它那种校准过的概率。
> 配置上少了一路接口：分析只认「模型接口」那一把 key。逐字段与它比对的结果见
> [`docs/EQUIVALENCE.md`](docs/EQUIVALENCE.md)。
>
> `apk/` 目录没有从它复制过来，这一份有自己的构建：[Releases 里的 `android-v0.1.0`](https://github.com/chy4pro/chat-nojev/releases/tag/android-v0.1.0)
> 是仓库自己的 CI 打出来的**未签名** APK——未签名的包装不上，要先自己签一次，见下面「快速开始」
> 第 1 条。**这个包没有在真机上跑过**，CI 只证明它编译打包成功；下面凡是提到
> 「原版」「它的」装包和真机实测的段落，说的都是 jev-chat-jarvis 那一份，不是这里的改版。
...
这一版用了自己的 application id（`com.jev.probe.nojev`）和自己的启动器名字（**对话副驾**），跟原版 jev-chat-jarvis 的包名不同，所以是**装在它旁边，不是替换它**——这正是目的：两个 App 能同时留在同一台手机上，直接对着比。
...
- **没有真机验证**：这一版在容器里编译、做了差分测试，CI 在 GitHub 的 runner 上把 `android-v0.1.0` 那个未签名 APK 打了出来（见「快速开始」），但没有人把它装到手机上跑过。CI 绿只说明编译打包成功，不说明装上去能用。
