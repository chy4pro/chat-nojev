...
> **这一份是 [jev-chat-jarvis](https://github.com/jev-chat/jev-chat-jarvis) 的派生（[chat-nojev](../README.md)），只改了一件事。**
> 原版把 7 道判断题和候选排序交给专门的判断模型（TypeSafe Jev），起草另算一次调用；这里把它们合成一次——
...
> 配置上少了一路接口，分析只认「模型接口」那一把 key。这道接缝有一套差分测试对着原版逐字段盯着，
> 判据和逐场景结果见 [`docs/EQUIVALENCE.md`](docs/EQUIVALENCE.md)——**那是关于这一处的证据，不是对这个
> App 的质量判断**。
>
> **其余全是原版的代码**：无障碍采集、各 App 适配器、OCR 兜底、知识库、起草的提示词、悬浮窗、
> 回填输入框、「不替你发送」那条边界，连同原版的 bug 和粗糙处一起留着。这一版没有审过这些代码。
> 「只改了一件事」不成立的几处列在下面[「不是原版的地方」](#不是原版的地方)。
>
> **这个包没有在真机上跑过**，CI 只证明它编译打包成功。下面凡是提到「原版」「它的」装包和真机实测的
> 段落（包括「为什么用它」那几条和平台支持表里的「实测」），说的都是 jev-chat-jarvis 那一份，
> 不是这里的改版。

## 不是原版的地方

合并本身带来的改动（删掉 `JudgeClient`、判断那一路的配置和设置页那张卡）不算，剩下的全在这里；
