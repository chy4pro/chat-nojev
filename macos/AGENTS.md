...
- **判断题面别瘦身**：上游两轮压缩措辞（保语义锚点）实测 81.8% / 77.3%，低于原文 86.4%——那是在 decider-2b 上测的，模型换了数字不再适用，但那几句描述现在是提示词的一部分，改之前先想清楚（见 `src/questions.py` 顶上的注释）。
- `.gitignore` 忽略全部 png 只放行 `docs/**`；新图片必须进 docs/。
- AppKit 控件宽度要渲染成 PNG 实测，`cellSize()` 会谎报。
- 首次调用偏慢是正常现象，别误判成回归：**这一版没有本地判断模型可预热**（那是上游 decider-2b 的坑），慢的是 Vision——OCR 首次加载比稳态贵约 2 倍，日志里首次那行会标注「首次」。
