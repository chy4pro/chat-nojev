...
#
# Why a launcher bundle instead of py2app/PyInstaller: upstream froze torch + transformers
# into a 2-4 GB app. This bundle stays ~100 KB: it carries the Python source and
# bootstraps a uv-managed virtualenv under ~/Library/Application Support on first launch.
# （这一版连 torch 都不装了——判断合进了那一次网络调用，本地没有模型要跑。）
#
...
# 上游这里还要 export USE_TF=0 / HF_HUB_DISABLE_TELEMETRY=1（transformers 和
# huggingface-hub 的开关）——本地判断模型没了，这两个依赖也没了。
...
# a key that leaked into src/ would ship to whoever gets the bundle. Upstream exempts
# src/builtin.py, where it ships its own relay's shared token on purpose. chat-nojev leaves
# that file empty (see src/builtin.py), so nothing is exempt: a key put back there fails the
# build like any other leak.
...
echo "    ✓ 没夹带 API key（builtin.py 也在检查范围内，这一版它是空的）"
