...
> **这一份是 [jev-chat-jarvis-mac](https://github.com/jev-chat/jev-chat-jarvis-mac) 的派生（[chat-nojev](../README.md)）。**
> 原版有两个模型：判断层（本地 `Mapika/decider-2b`，或云端 TypeSafe Jev）出意图和风险、再单独问一次
> 「哪条回复最合适」，生成层只写候选。这里合成一次：写候选的模型顺手答那两道判断题、写「具体行动」、
> 并给自己写的每条候选打分。题面、8 类意图的描述、0–9 的风险判据一个字没改（见 `src/questions.py`），
> 面板读到的字段也一模一样，**换的只是产出方**。代价里最重的一处：**判断不再能只在本地跑了**，见下面
> [「不是原版的地方」](#不是原版的地方)。其余全是原版的代码，连同它的 bug；这一版既没有审过它，
> 也没有 Mac 可以跑它。差分测试的判据、逐场景结果和 5 处有意为之的分歧见
...
合并本身带来的改动（删掉判断层的客户端、`TYPESAFE_*` 配置和设置页那一页）不算，剩下跟上游的出入全在
这里；根 [README](../README.md#和原版不一样的地方) 里三个变体一起列，长版在 [`../docs/MERGE.md`](../docs/MERGE.md#macos)。
