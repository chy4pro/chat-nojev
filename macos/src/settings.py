...
class SettingsController(NSObject):
    ...
    def build(self, hud=None):
        ...
        title = self.label(view, "模型与会话设置", 24, 550, 710, 28, 22)
        ...
        # 上游是三页：「判断 · Jev」+ 两页生成。判断合进了生成那一次调用，
        # 判断层那一页没有对应的配置了（config.PREFIXES 也少了 TYPESAFE）。
        titles = ("OpenAI 兼容", "Anthropic 兼容")
        for index, (prefix, title) in enumerate(zip(config.PREFIXES, titles)):
            ...
            if prefix == "OPENAI":
                ...
                self.label(panel, "使用 Anthropic 消息接口，支持自定义兼容服务地址。",
                           26, 43, 638, 20, 11, PALETTE["muted"])
        ...
        # 上游这里还有一节「离线判断模型」（#38：显示占用、删除、启用后下次启动预热时下载）。
        # 判断合进了生成那一次调用，没有本地模型可管——整节连带它的按钮和动作一起没了，
        # 窗口高度也退回上游加这一节之前的 600（上面那几个 y 坐标同样退回去）。
