...
The names are the conventional ones you likely already export for other tools:
    ...
    OPENAI_API_KEY       the one call (candidates + intent/risk), any OpenAI-compatible
                         endpoint
...
上游还有一组 TYPESAFE_*（判断层 TypeSafe Jev 的 key / 地址 / 模型，不填就走本地
decider-2b）。判断合进了生成那一次调用，那一组没有对应物了——**全程只有这一把 key**。
"""
