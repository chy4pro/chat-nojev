...
# hud.py 的模块级兜底读取函数：适配层只翻译不校验，所以判断值的兜底住在这几个函数里
# （src/hud.py 顶部注释）。跟方法一起 AST 载进来，测的就是线上那一份。
GUARDS = {'_judgment_text', '_risk_level', '_confidence_text', '_prob_text', '_actions_text'}
...
def hud_harness():
    ...
    guards = [n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name in GUARDS]
    ...
    scope = {'chat_context': chat_context, 'styles': styles, 'isfinite': isfinite,
    ...
    module = ast.fix_missing_locations(ast.Module(body=guards + [klass], type_ignores=[]))
