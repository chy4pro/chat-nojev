"""HUD reply-pipeline regressions: reply epochs/keys, incoming gating, the early-run
slot, and foreground/read-failure handling. Run: python -B -m unittest
discover -s tests -v.

上游这里的断言有一半是「预判槽位」（_prejudge_req/_result）：判断是独立一次调用，所以
它有自己的队列。判断合进了候选那一次调用之后只剩早跑一条线（_pregen_*），所有原来查
两个槽位的用例改成查这一个——测的行为（谁排队、谁作废、谁上屏）一个都没少。
...
class HudReplyTests(unittest.TestCase):
    def setUp(self):
        ...
        for name, value in dict(
            ...
            _gen_epoch=0, _last_skip_reason=None,
            _pregen_req=None, _pregen_result=None, _pregen_running=False,
            _paused=False, _analyzing=False, _regenerating=False,
    ...
    def test_all_model_paths_share_twenty_messages_including_target(self):
        # 上游这里先核对「预判和早跑拿到的是同一份上下文」，再核对换话术和排序调用也拿它。
        # 判断和排序都在那一次调用里，所以只剩一条路要核对：早跑排的和换话术发的是同一份。
    ...
    def test_external_clear_retires_old_context_and_late_model_results(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text('')
            self.h._reply_task(old_epoch, self.h._regen_work, '当前消息', ['normal'])
            self.h.generator.generate.assert_not_called()
            self.h._analyzing = True
            # 上游这里用的是 _run_generation（预判命中那半路）；合成一次之后只有 _run_analysis，
            # 同一个 finally 负责放开分析闸
            self.h._reply_task(old_epoch, self.h._run_analysis, messages[-1], messages, '')
            ...
            self.h._work_inner()
            self.assertNotIn('合成旧历史', self.h._pregen_req[1])
            self.assertNotIn('合成旧背景', self.h._pregen_req[1])
    ...
    def test_request_preserves_current_message_that_fits_budget(self):
        ...
        self.h._reply_task(self.h._reply_epoch, self.h._regen_work, text, ['normal'])
    ...
    def test_invalidated_generation_does_not_paint_old_context_candidates(self):
        # 上游断言的是「不会再发一次用旧上下文的排序调用」。排序没有自己的调用了，
        # 所以改断言同一件事的下一环：那批候选一个字都不上屏。
        self.incoming()
        ...
        self.h.applyTones_ = Mock()
        self.h._reply_task(self.h._reply_epoch, self.h._regen_work, '下午开会', ['normal'])
        self.flush()
        self.h.applyTones_.assert_not_called()
        self.h.applyCandidates_.assert_not_called()
    ...
    def test_real_incoming_triggers_the_run_and_keeps_own_context(self):
        # 上游这里是两个队列（预判 + 早跑各一个）；判断合进那一次调用之后只剩早跑
        self.read([block('下午开会', .40, .70, .15), block('我会带材料', .78, .50, .10)])
        self.assertEqual(self.h._pregen_req[0], '下午开会')
        self.assertIn('我: 我会带材料', self.h._pregen_req[1])
        self.flush()
    ...
    def test_empty_ocr_also_invalidates_target(self):
        ...
        # in-flight worker and re-arm the prework at the new epoch — a candidate
    ...
    def test_empty_frame_streak_reuses_without_resetting_settle(self):
        ...
        self.assertEqual(self.h._pregen_req[3], epoch + 1)
    ...
    def test_old_generation_does_not_restart_or_paint(self):
        self.incoming()
        epoch = self.h._reply_epoch
        self.read([])
        self.h._reply_task(epoch, self.h._gen_with_pregen, '下午开会', None)
        self.h.generator.generate.assert_not_called()
        # 上游这里断言「不会再发一次排序调用」；排序没有自己的调用了，改断言它一个字都不上屏
        self.h._reply_task(epoch, self.h._finish_generate, {}, time.perf_counter())
        self.flush()
        self.h.applyCandidates_.assert_not_called()
    ...
    # 上游这里还有 test_prejudge_completion_cannot_repopulate_cleared_state：预判线程回来
    # 得晚也不许把已清空的槽位填回去。判断合进了这一次调用，那条线程没了——同一条纪律由
    # 下面这个早跑版用例守着（两者原本是逐字对称的）。
    ...
    def test_fill_candidate_without_app_reports_not_frontmost(self):
        ...
        self.h._render.assert_any_call(
            'status', '填入失败：聊天应用不在前台', HUD['PALETTE']['red'])
