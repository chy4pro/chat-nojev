...
- `gradlew` 补上了可执行位；版本号不跟原版，从 `0.1.0` 起自己编（现在 `0.2.0`）；`apk/` 目录没有跟着拷过来。
...
- 输出预算 300 → 900 + 150 × 每话术候选数（默认 2 条即 1200）；版本号不跟原版（现在 `0.2.0`）；`torch` / `transformers` / `huggingface-hub` / `laya` 从依赖删了，但 `uv.lock` 是原版 0.6.0 那一份，还锁着它们——**已知的不一致**。
